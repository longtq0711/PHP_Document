      ...
      //// - Xử lý submit form
            if (isset($_POST['submit'])){
                ...
                if (empty($name)||empty($description)){
                    $this->error = 'Ko được để trống';
                }
                //Lưu vào bảng categories chỉ khi k có lỗi xảy ra
                if (empty($this->error)){
                    //Gọi Model để lưu vào CSDL, cần nhúng model vào, khởi tạo đối tượng từ model này để có thể
                    //sử dụng được thuộc tính/phương thức
                    $category_model = new Category();
                    //Gán giá trị từ form cho thuộc tính của model, vì phương thức insert dữ liệu đang thao tác với thuộc
                    //tính của chính model đó
...
>>>>>>> e67be450aed7962e444e31af6a2039171ec696fa
...
<<<<<<< HEAD
  public function index(){
      //Gọi model để lấy tất cả danh mục, truyền ra view để view hiển thị (MVC)
      $category_model = new Category();
      ...
      //Bước đầu tiên khi xây dựng 1 chức năng mới là hiển thị ra view
      // - Lấy ra nội dung view
      $this->content = $this->render('views/categories/index.php',$arr);
      // Gọi layout vào để hiển thị view vừa lấy được
      require_once 'views/layouts/main.php';
  }
}
?>
