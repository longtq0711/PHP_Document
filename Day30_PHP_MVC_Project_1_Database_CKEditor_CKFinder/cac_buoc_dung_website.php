...
<<<<<<< HEAD
 * Phân tích từng trang HTML-> đi từ trên xuống dưới xem các vùng hiển thị có cần phải lưu lại trong SDL không
 ...
 *      created_at: ngày tạo, TIME STAMP, CURRENT_TIMESTAMP
 ...
 *      title: tên sản phẩm, VARCHAR(150)
 *      category_id: khóa ngoại, liên kết với bảng categories, thể hiện cho mối quan hệ: 1 sản phẩm chỉ thuộc về 1 danh
 *      mục nào đó
 ...
 *      avatar: ảnh danh mục, VARCHAR(100)
 ...
 *      email
 *      address
 *      role_id: khóa ngoại, liên kết với bảng roles, 1 user chỉ có 1 quyền duy nhất
 *      .....
 *  + Bảng roles: lưu các quyền trên hệ thống
 *      id, status, created_at, updated_at
 *      name: tên quyền: super admin, sale, editor, user
 *  + Bảng orders: lưu thông tin đơn hàng
 *      id,
 *      status: 0 - chưa thanh toán, 1 - đã thanh toán, 2 - đang trả góp, 3 - đang giao hàng,.. TINYINT(1)
 *      created_at
 *      updated_at
 *      user_id:: tên ng mua hàng
 *      addres: địa chỉ ng mua
 *      moblie: sđt người mua
 *      email
 *      note:
 *      price_total: tổng giá trị đơn hàng INT(11)
 *  + Bảng order_details: lưu thông tin chi tiết đơn hàng: mua bao nhiêu sản phẩm, mỗi sp có số lượng bao nhiêu
 *      order_details
 *      id
 ...
 * 4 - Sau khi tạo CSDL và tạo các bảng thành công, vào tab Design của PHPMyAdmin, khi làm tài liệu lúc bảo vệ đồ án,
 * nhớ chụp lại thông tin trong tab này
 * 5 - Phân tích 1 số chức năng thực tế của 1 website bán hàng để các bạn tự tư duy làm chức năng này
 *  + Backend/Admin:
 *     - QL SP, News, Category, User -> CRUD
 *     - QL đơn hàng: xem đơn hàng
 *      a, Sửa đơn hàng? cho phép sửa thông tin cơ bản của ng mua hàng, ko cho phép sửa thông tin chi tiết của đơn hàng
 *      b, In hóa đơn: đọc nội dung đơn hàng, lưu vào 1 file .pdf
 *      c, Xóa đơn hàng? cho phép xóa, lưu lại vào log của ng đã xóa
 *      d, Cập nhật trạng thái đơn hàng: dựa vào trạng thái thanh toán của đơn hàng
 *     - Thống kê:
 *      a/ Sp nào bán chạy nhất: dựa vào số lượng sản phẩm đã bán
 ...
 *     - Đánh giá sản phẩm: yêu cầu đăng nhập mới cho phép đánh giá CSDL, tạo bảng votes:
 *      id
 *      comment: nội dung đánh giá
 *      user_id: id của user đánh giá, khóa ngoại
 *      star: số sao đánh giá, tham khảo các plugin của JS, tạo 1 field ẩn trong form, khi user click chọn số sao
 *      dùng JS để set giá trị tương ứng cho input đang ẩn đó
 *     - Sản phẩm yêu thích: có thể lưu các sản phẩm yêu thích vào CSDL hoặc ko, lưu vào COOKIE
 *      Bảng favorites:
 *      id
 *      product_id: id của sản phẩm yêu thích
 *      user_id: id của user thích sản phẩm, cho phép rỗng
 *     - Tích hợp chat trực tuyến:
 *      Facebook: search tích hợp Message của Facebook vào, có thể tích hợp cả comment Facebook
 ...
 *     - Tự động hiển thị thong tin user mua hàng ở trang Thanh toán khi user đã login
 ...
 * Sau đó bên Frontend sẽ lấy dữ liệu này để hiển thị v
 *   - Với website demo, cấu trúc thư mục theo mô hình MVC sẽ có dạng sau: Cả 2 đều chứa cấu trúc thư mục mvc giống nhau
 *      backend/
 *      frontend/
 *   - Với backend, tham khảo cấu trúc thư mục đã dựng sẵn theo MVC đã học tại Code_demo tren lop/mvc_demo/backend
 *   - Code file index.php trước
 *
 */
=======
 ...
 *
 */
>>>>>>> ec222bf2770ad2218f192d55b7ab63e2e3ca0ab9
